...
public class ClassPathScanHandler {
	 ...
	 * @return Set
	 */
	public List<Class<?>> getPackageAllClasses(String basePackage, boolean recursive) {
		...
		try {
			...
			while (dirs.hasMoreElements()) {
				...
				if ("file".equals(protocol)) {
					...
					logger.info("扫描jboss服务器上指定包["+basePackage+"]的类");
					doScanPackageClassesByVfs(packageName, url, recursive, classes);
				}
			}
		} catch (IOException e) {
	 ...
	 * @throws IOException 
	 */
	private void doScanPackageClassesByVfs(String packageName, URL url, boolean recursive, List<Class<?>> classes) throws IOException {
		URLConnection conn = url.openConnection();
		VirtualFile packageVf = (VirtualFile)conn.getContent();
		if(packageVf == null || !packageVf.isDirectory()){
			logger.error("The package ["+packageName+"] VirtualFile is null or is not directory!");
			return;
		}
		List<VirtualFile> childrenVfs = null;
		if(recursive)
			childrenVfs = packageVf.getChildrenRecursively();
		else
			childrenVfs = packageVf.getChildren();
		
		for (VirtualFile vf : childrenVfs) {
	 ...
	 * @param classes
	 */
	private void doScanPackageClassesByJar(String basePackage, URL url, final boolean recursive, List<Class<?>> classes) {
		String packageName = basePackage;
		String package2Path = packageName.replace('.', '/');
		JarFile jar;
		try {
	 ...
	 * @param classes
	 */
	private void doScanPackageClassesByFile(List<Class<?>> classes, String packageName, String packagePath,
	    boolean recursive) {
	...
	public static void main(String[] args) {
		...
		List<Class<?>> calssList = handler.getPackageAllClasses("com.signavio.editor.handler", false);
